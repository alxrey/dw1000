 ...
 *
 *           This is a simple code example that turns on the DW1000 receiver to 
 *           receive a frame, (expecting the frame as sent by the companion 
 *           simple example "ACK DATA TX"). The DW1000 is configured so that 
 *           when a correctly addressed data frame is received with the ACK 
 *           request (AR) bit set in the frame control field, the DW1000 will 
...
int main(void)
{
    ...
    while (1) {
        ...
        if (status_reg & SYS_STATUS_RXFCG) {
            ...
            /* Since the auto ACK feature is enabled, an ACK should be sent if
             * the received frame requests it, so we await the ACK TX 
 ...
 * NOTES:
 *
 * 1. PAN ID, EUI and short address are hard coded constants to keep the 
 *    example simple but for a real product every device should have a 
 *    unique ID.
 *    For development purposes it is possible to generate a DW1000 unique ID 
 *    by combining the Lot ID & Part Number values programmed into the DW1000
 ...
 * 2. EUI64 is not actually used in this example but the DW1000 is set up with 
 *    this dummy value, to have it set to something. This would be required
 *    for a real application, i.e. because short addresses (and PAN ID) are 
 *    typically assigned by a PAN coordinator.
 * 3. In this example, maximum frame length is set to 127 bytes which is 
 ...
 *    example.
 * 4. In this example, LDE microcode is not loaded upon calling 
 ...
 * 6. Manual reception activation is performed here but DW1000 offers several 
 ...
 * 8. This is the purpose of the AAT bit in DW1000's STATUS register but 
 *    because of an issue with the operation of AAT, it is simpler to directly
 *    check in the frame control if the ACK request bit is set. Please refer 
 *    to DW1000 User Manual for more details on Auto ACK feature and the AAT
 *    bit.
