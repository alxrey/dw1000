...
int main(void)
{
     ...
     * Since DW1000 is not woken by the reset line, we could get here with it
     * asleep. Note that this may be true in other examples but we pay special
    ...
    /* Configure DW1000 to automatically enter sleep mode after 
    ...
    while (1) {
        ...
        /* It is not possible to access DW1000 registers once it has sent the 
         * frame and gone to sleep, and therefore we do not try to poll for TX
         * frame sent, but instead simply wait sufficient time for the DW1000
         * to wake up again before we loop back to send another frame.
         * If interrupts are enabled, (e.g. if MTXFRS bit is set in the 
         * SYS_MASK register) then the TXFRS event will cause an active 
         * interrupt and prevent the DW1000 from sleeping.
         */
