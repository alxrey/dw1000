...
// static const dwt_hi_cfg dwt_hi_cfg_0 = {
//     .irq_gpio = GPIO_DT_SPEC_INST_GET(0, int_gpio);
//     .rst_gpio = GPIO_DT_SPEC_INST_GET(0, rst_gpio);
// };
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
    ...
    diagnostics->maxGrowthCIR = dwt_read16bitoffsetreg(RX_FQUAL_ID, RX_FQUAL_CIR_PWR_OFFSET);
}
...
void _dwt_loaducodefromrom(void)
{
    ...
    k_msleep(1); // Allow time for code to upload (should take up to 120 us)
...
uint16_t dwt_readtempvbat(uint8_t fastSPI)
{
    ...
    if(fastSPI == 1)
    {
        ...
        k_msleep(1); // If using PLL clocks(and fast SPI rate) then this sleep is needed
 ...
 * output parameters:
 *
 * returns: (uint32_t) The setting to be programmed into the TX_POWER register
 */
