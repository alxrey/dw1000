 ...
 *
 *           This is a simple code example that turns on the DW1000 receiver 
 *           to receive a frame, (expecting the frame as sent by the companion 
 ...
 *    is required, DWT_LOADUCODE parameter should be used. See two-way ranging 
 *    examples (e.g. examples 5a/5b).
 ...
 *    the dwt_configuretxrf API call) to per device calibrated values saved in the
 ...
 *    optimize system's overall performance (e.g. timeout after a given time, 
 ...
 * frame (that will be ignored anyway) following the same standard. The
 ...
 *                   keep it simple.
 *     - byte 15: function code (0x10 to indicate this is an activity control 
 *                message).
 ...
 */
static uint8_t tx_msg[] = {0x41, 0x8C, 0, 0x9A, 0x60, 0, 0, 0, 0, 0, 0, 0, 0, 
                         'D', 'W', 0x10, 0x00, 0, 0, 0, 0};

/* Indexes to access to sequence number and destination address of the data
 * frame in the tx_msg array.
 */
...
/* Index to access to source address of the blink frame in the 
 * rx_buffer array.
 */
...
int main(void)
{
    ...
    while (1) {
         ...
         * See NOTE 5 below.
         * STATUS register is 5 bytes long but, as the events we are 
         * looking at are in the lower bytes of the register, we can use 
        ...
        if (status_reg & SYS_STATUS_RXFCG) {
            ...
            /* Validate the frame is the one expected as sent by "TX then wait 
             * for a response" example. 
             */
            if ((frame_len == 14) && (rx_buffer[0] == 0xC5) && 
                (rx_buffer[10] == 0x43) && (rx_buffer[11] == 0x2)) {
