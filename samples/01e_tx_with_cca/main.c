 ...
 * 
 * @note
 * 1. For Wireless Sensor Networks application, most of the MAC protocols rely 
 *    on Clear Channel Assessment (CCA) to avoid collisions with other frames
 ...
 *    approach is to just look for preamble to avoid conflicting transmissions,
 *    since any sending of preamble during data will typically not disturb those 
 *    receivers who are demodulating in data mode.
 *    The idea then is to sample the air for a small amount of time to see if a 
 *    preamble can be detected, then if preamble is not seen the transmission
 *    is initiated, otherwise we defer the transmission typically for a random 
 *    back-off period after which transmission is again attempted with CCA.
 *    Note: we return to idle for the back-off period and do not receive the 
 *    frame whose preamble was detected, since the MAC (and upper layer) wants
 *    to transmit and not receive at this time.
 *    This example has been designed to operate with example 4b - Continuous 
 *    Frame. The 4b device will fill the air with frames which will be detected 
 *    by the CCA and thus the CCA will cancel the transmission and will use back
 *    off to try sending again at later stage.
 *    This example will actually get to send when the CCA preamble detection 
 *    overlaps with the data portion of the continuous TX or inter frame period,
 *    Note the Continuous Frame example actually stops after 30s interval (thus 
 *    the user should toggle the reset button on the unit running example 4b
 *    to restart it if they wish to continue observing this pseudo CCA 
 *    experiencing an environment of high air-utilisation).
 * 2. The device ID is a hard coded constant in the blink to keep the example 
 ...
 *    dwt_initialise(). This will prevent the IC from generating an RX 
 *    timestamp. If time-stamping is required, DWT_LOADUCODE parameter should 
 *    be used. See two-way ranging examples (e.g. examples 5a/5b).
 * 4. In a real application, for optimum performance within regulatory limits, 
 ...
 * 5  The preamble timeout of 3 PACs is recommended as sufficient for this CCA 
 *    example for all modes and data rates. The PAC size should be different
 *    when is different for different preamble configurations, as per User 
 *    Manual guidelines.
 * 6. dwt_writetxdata() takes the full size of tx_msg as a parameter but only 
 ...
 *    possible but the RXPRD and RXPTO status events can be used to generate 
 *    an interrupt.
 ...
 *    Please refer to DW1000 User Manual for more details on "interrupts".
 * 9. The user is referred to DecaRanging ARM application (distributed with 
...
/* initial backoff period when failed to transmit a frame due to 
 * preamble detection
 */
#define INITIAL_BACKOFF_PERIOD 400 /* This constant would normally smaller 
                                    * (e.g. 1ms), however here it is set to 
                                    * 400 ms so that user can see (on console)
                                    * the report that the CCA detects a 
                                    * preamble on the air occasionally.
                                    */

int tx_sleep_period; /* Sleep period until the next TX attempt */

/* Next backoff in the event of busy channel detection by this 
 * pseudo CCA algorithm
 */
...
int main(void)
{
     ...
     * time we assume channel is clear. See Note 5
     */
    ...
    while(1) {
        ...
        /* local variables for console output, this holds the result of the
         * most recent channel assessment and pseudo CCA algorithm,
        ...
        /* Start transmission. Will be delayed (the above RX command has 
         * to finish first) until we get the preamble timeout or canceled by
         * TRX OFF if a preamble is detected.
         */
        dwt_starttx(DWT_START_TX_IMMEDIATE);

        /* Poll DW1000 until preamble timeout or detection. See NOTE 7 below. */
        while (!((status_reg = dwt_read32bitreg(SYS_STATUS_ID)) & 
               (SYS_STATUS_RXPRD | SYS_STATUS_RXPTO)));
        ...
        if (status_reg & SYS_STATUS_RXPTO) {
             ...
             * In a real implementation the back-off is typically a randomised
             * period whose range is an exponentially related to the number 
        ...
        /* Note in order to see cca_result of 0 on the console, the backoff 
         * period is artificially set to 400 ms
         */
        sprintf(console_str, "CCA=%d   %d\n", channel_clear, tx_sleep_period);
        printk("%s", console_str);

        /* Execute a delay between transmissions. */
        k_msleep(tx_sleep_period);
