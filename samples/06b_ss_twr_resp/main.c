 ...
 *    sensitive to the clock offset error between the devices and the length of
 *    the response delay between frames. To achieve the best possible accuracy,
 *    this response delay must be kept as low as possible. In order to do so,
 *    6.8 Mbps data rate is used in this example and the response delay between
 *    frames is defined as low as possible. The user is referred to User Manual
 *    for more details about the single-sided two-way ranging process.
 ...
 *    value but, in a real application, each device should have its own antenna
 *    delay properly calibrated to get the best possible precision when
