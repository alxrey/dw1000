 ...
 *
 *           This is a simple code example that sends a frame and then turns 
 *           on the DW1000 receiver to receive a response, expected to be an 
 *           ACK frame as sent by the companion simple example "ACK DATA RX" 
 *           example code. After the ACK is received, this application proceeds 
 *           to the sending of the next frame (increasing the frame sequence 
 *           number). If the expected valid ACK is not received, the application
 *           immediately retries to send the frame (keeping the same frame 
 *           sequence number).
 *
 * @copyright
 ...
 * IEEE 802.15.4-2011 standard. It is a 21-byte frame composed of the following
 * fields:
 *     - byte 0/1: frame control (0x8861 to indicate a data frame using 16-bit 
 *                 addressing and requesting ACK).
 ...
 */
static uint8_t tx_msg[] = {0x61, 0x88, 0, 0xCA, 0xDE, 'X', 'R', 'X', 'T', 'm', 
                         'a', 'c', 'p', 'a', 'y', 'l', 'o', 'a', 'd', 0, 0};

/* Index to access the sequence number and frame control fields in frames 
 * sent and received.
 */
 ...
 * (UUS, 1 uus = 512/499.2 �s). See NOTE 3 below.
 */
...
int main(void)
{
    ...
    /* Set delay to turn reception on immediately after transmission of 
     * the frame. See NOTE 6 below. 
     */
    ...
    while (1) {
        ...
        if (status_reg & SYS_STATUS_RXFCG) {
            ...
            /* A frame has been received, check frame length is correct for 
             * ACK, then read and verify the ACK.
             */
 ...
 * NOTES:
 *
 * 1. This example can be turned into a high speed data transfer test by 
 *    removing the delay executed between two successful transmissions. The
 *    communication configuration and MAC payload size in the message can be 
 *    changed to see the effect of the different parameters on the throughput
 *    (which can be computed using the different counters provided in the 
 *    application). For example using the debugger to stop at the start of the
 *    while loop, and then timing from the "GO" for a few minutes before 
 *    breaking in again, and examining the frame counters.
 * 2. Source and destination addresses are hard coded constants to keep the 
 *    example simple but for a real product every device should have a unique ID.
 ...
 *    time to receive a complete ACK frame sent by the "ACK DATA RX" example
 *    at the 110k data rate used (around 2 ms).
 * 4. In this example, receive buffer is set to the exact size of the only 
 ...
 * 5. In a real application, for optimum performance within regulatory limits, 
 ...
 * 6. TX to RX delay is set to 0 to activate reception immediately after 
 *    transmission, as the companion "ACK DATA RX" example is configured to send
 *    the ACK immediately after reception of the frame sent by this example 
 *    application.
 * 7. dwt_writetxdata() takes the full size of tx_msg as a parameter but only 
 *    copies (size - 2) bytes as the check-sum at the end of the frame is
 *    automatically appended by the DW1000. This means that our tx_msg could be
 *    two bytes shorter without losing any data (but the sizeof would not
